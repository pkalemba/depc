...
class ConfigController(Controller):
    ...
    def _create_context_from_config(cls, config):
        ...
        regex_patterns = {
            "rule": r"^rule.(.+|'.+')$",
            "operation":
                r"^operation.(ATLEAST|RATIO|AND|OR)\(?[0.|0-9]*\)?(\[[A-Z]+[a-zA-Z0-9]*(, [A-Z]+[a-zA-Z0-9]*)*?\])$",
            ...
            for compute_type, regex in regex_patterns.items():
                ...
                if match:
                    try:
                        ...
                        rule_name = rule_name[1:-1] if rule_name.startswith("'") else rule_name
    ...
    def _check_config_from_context(cls, config_context, team_id):
        ...
        for label_name, meta in config_context['all_labels'].items():
            ...
            if meta['compute_type'] == 'rule' \
                    and config_context['all_labels'][label_name]['rule_name'] not in already_checked_rules:
            ...
            elif meta['compute_type'] == 'operation' or meta['compute_type'] == 'aggregation':
                for dep in meta['dependencies']:
                    if dep not in config_context['all_labels'].keys():
                        msg = ('Dependency "{dep}" declared in label "{label}" has not '
